...
public class EntityConfig {
    ...
    void bake() {
        Map<String, Biome.Category> categoryByName = Arrays.stream(Biome.Category.values()).collect(Collectors.toMap(Biome.Category::getName, category -> category));
        if (biomeCategories.get().contains("*")) {
            worldGenBiomeCategories = new HashSet<>(Arrays.asList(Biome.Category.values()));
        } else {
            worldGenBiomeCategories = biomeCategories.get().stream().map(categoryByName::get).collect(Collectors.toSet());
        }
    ...
    public boolean canSpawnIn(@Nullable Biome biome) {
        if (weight.get() <= 0 || biome == null) {
            return false;
        }
        ResourceLocation registryName = biome.getRegistryName();
        return registryName != null && worldGenBiomeCategories.contains(biome.getCategory()) && !worldGenBiomeBlacklist.contains(registryName) && !worldGenModIdBlacklist.contains(registryName.getNamespace());
    }
}
